  ...
  if (loading && projects.length === 0) {
    return (
      <section className="py-24 lg:py-32 bg-white">
        <div className="flex justify-center"><Loader2 className="w-6 h-6 text-[#0A7A70] animate-spin" /></div>
      </section>
    )
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#0A7A70]/10 border border-[#0A7A70]/20 mb-6">
            <Sun className="w-3.5 h-3.5 text-[#0A7A70]" />
            <span className="font-inter text-xs font-semibold text-[#0A7A70] tracking-wider uppercase">Our Projects</span>
          </div>
          <h2 className="font-jakarta font-extrabold text-[#0F172A] text-4xl lg:text-5xl tracking-tight mb-4">
            Recently <span className="text-[#0A7A70]">Completed</span>
          </h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map(p => (
            <Link key={p.id} to={`/project/${p.id}`} className="group bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden hover:shadow-xl hover:border-[#0A7A70]/20 transition-all duration-500 block">
              <div className="relative h-52 overflow-hidden bg-[#F1F5F9]">
                {p.images?.[0] ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Sun className="w-12 h-12 text-[#0A7A70]/20" />
                  </div>
                )}
                {p.systemType && (
              ...
              <div className="p-6">
                <h3 className="font-jakarta font-bold text-[#0F172A] text-base mb-2 line-clamp-2 group-hover:text-[#0A7A70] transition-colors">
                  {p.title}
                </h3>
                ...
                <div className="mt-4 pt-4 border-t border-[#E2E8F0]">
                  <span className="inline-flex items-center gap-1 text-sm font-inter font-medium text-[#0A7A70] group-hover:gap-2 transition-all">

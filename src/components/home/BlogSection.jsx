        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-14 gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1E3A5F]/10 border border-[#1E3A5F]/20 mb-5">
              ...
              <span className="font-inter text-xs font-semibold text-[#1E3A5F] tracking-wider uppercase">Solar Knowledge</span>
            </div>
            <h2 className="font-jakarta font-extrabold text-[#0F172A] text-4xl lg:text-5xl tracking-tight">
              Learn About <span className="text-[#1E3A5F]">Solar Energy</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {display.slice(0, 6).map((post, i) => (
              <article key={post.id} className={`group bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden card-hover flex flex-col ${i === 0 ? 'md:col-span-2 lg:col-span-1' : ''}`}>
                {/* Image or gradient */}
                <div className="h-48 bg-gradient-to-br from-[#1E3A5F]/10 via-[#F8FAFC] to-[#D97706]/5 flex items-center justify-center relative overflow-hidden">
                  <BookOpen className="w-12 h-12 text-[#1E3A5F]/20" />
                  <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-[#1E3A5F]/10 border border-[#1E3A5F]/20">
                    <span className="font-inter text-xs font-semibold text-[#1E3A5F]">Solar Guide</span>
                  </div>
                </div>
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="font-jakarta font-bold text-[#0F172A] text-base leading-snug mb-3 group-hover:text-[#1E3A5F] transition-colors duration-300">
                    {post.title}
                  </h3>
                  <p className="font-inter text-sm text-[#475569] leading-relaxed flex-1 mb-5">{post.shortDescription}</p>
                  <Link
                    to={`/blog/${post.slug || post.id}`}
                    className="inline-flex items-center gap-2 text-sm font-jakarta font-semibold text-[#1E3A5F] hover:text-[#D97706] transition-colors duration-300 group/link"
                  >
                    Read Article

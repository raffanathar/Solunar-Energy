...
import { Sun, ArrowRight, Loader2, MessageCircle } from 'lucide-react';
...
function PackageCard({ pkg }) {
  ...
  return (
    <div className={`relative group bg-white rounded-2xl border-2 p-6 card-hover flex flex-col ${
      pkg.highlight
        ? 'border-[#0A7A70] shadow-xl shadow-[#0A7A70]/10'
        : 'border-[#E2E8F0]'
    }`}>
      {pkg.highlight && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="px-4 py-1 rounded-full bg-[#0A7A70] text-white text-xs font-jakarta font-bold shadow-md whitespace-nowrap">
            Most Popular
          </span>
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <div className="font-jakarta font-bold text-base text-[#0A7A70]">{pkg.name}</div>
        </div>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="font-jakarta font-bold text-xs text-[#0A7A70]">{pkg.coveragePercent}%</span>
        </div>
        <div className="h-2 bg-[#F1F5F9] rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-[#0A7A70] to-[#D97706] rounded-full transition-all duration-1000"
      ...
      <ul className="space-y-2 mb-6 flex-1">
        {pkg.components.map((c, i) => (
          <li key={i} className="flex items-center gap-2 text-sm font-inter text-[#475569]">
            <span className="w-1.5 h-1.5 rounded-full bg-[#0A7A70] flex-shrink-0" />
            {c}
          </li>
        ))}
      </ul>
      ...
      <div className="space-y-2">
        ...
        <a
          href={`https://wa.me/923214407701?text=${whatsappMsg}`}
          target="_blank"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="font-jakarta font-extrabold text-[#0F172A] text-4xl lg:text-5xl tracking-tight mb-5">
            Choose Your <span className="text-[#0A7A70]">Solar System</span>
          </h2>

...
const defaultServices = [
  { id: 'd1', icon: 'Home', title: 'Residential Solar Installation', description: 'Complete rooftop solar solutions for homes. Reduce your WAPDA bills by up to 90% with our premium residential packages.' },
...
function ServiceCard({ service }) {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-[#E2E8F0] p-6 card-hover flex flex-col">
      <div className="w-12 h-12 rounded-xl bg-[#0A7A70]/10 flex items-center justify-center mb-5 group-hover:bg-[#0A7A70] transition-colors duration-500">
        <IconComp className="w-6 h-6 text-[#0A7A70] group-hover:text-white transition-colors duration-500" />
      </div>
      ...
      <a
        href="#quote"
        className="inline-flex items-center gap-2 text-sm font-jakarta font-semibold text-[#0A7A70] hover:text-[#D97706] transition-colors duration-300 group/btn"
      >
        Request Quote
...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="py-24 lg:py-32 bg-[#F8FAFC] section-rule relative overflow-hidden">
      <div className="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-[#0A7A70]/5 to-transparent rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#0A7A70]/10 border border-[#0A7A70]/20 mb-6">
            <span className="font-inter text-xs font-semibold text-[#0A7A70] tracking-wider uppercase">Our Services</span>
          </div>
          <h2 className="font-jakarta font-extrabold text-[#0F172A] text-4xl lg:text-5xl tracking-tight mb-5">
            Complete Solar Solutions <br />
            <span className="text-[#0A7A70]">For Every Need</span>
          </h2>

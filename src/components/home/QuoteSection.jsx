...
import { ArrowRight, ArrowLeft, CheckCircle, Loader2, Zap } from 'lucide-react';
    ...
    return (
      <section id="quote" className="py-24 lg:py-32 bg-white section-rule">
        <div className="max-w-2xl mx-auto px-4 text-center">
          <div className="w-20 h-20 rounded-full bg-[#0A7A70]/10 flex items-center justify-center mx-auto mb-6">
            <CheckCircle className="w-10 h-10 text-[#0A7A70]" />
          </div>
          ...
          <button
            ...
            className="px-6 py-3 rounded-full border-2 border-[#0A7A70] text-[#0A7A70] font-jakarta font-semibold text-sm hover:bg-[#0A7A70]/5 transition-colors"
          >
  ...
  return (
    <section id="quote" className="py-24 lg:py-32 bg-white section-rule relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-80 h-80 bg-gradient-to-bl from-[#0A7A70]/5 to-transparent rounded-full blur-3xl" />
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#0A7A70]/10 border border-[#0A7A70]/20 mb-6">
            ...
            <span className="font-inter text-xs font-semibold text-[#0A7A70] tracking-wider uppercase">Free Quotation</span>
          </div>
          <h2 className="font-jakarta font-extrabold text-[#0F172A] text-4xl lg:text-5xl tracking-tight mb-4">
            Get Your Free <span className="text-[#0A7A70]">Solar Quote</span>
          </h2>
        ...
        <div className="flex items-center gap-3 mb-10 max-w-sm mx-auto">
          {[1, 2, 3].map(s => (
            <div key={s} className="flex-1 relative">
              <div className={`h-1.5 rounded-full transition-all duration-500 ${s <= step ? 'bg-[#0A7A70]' : 'bg-[#E2E8F0]'}`} />
              <div className={`absolute -top-1 left-1/2 -translate-x-1/2 w-3.5 h-3.5 rounded-full border-2 transition-all duration-500 ${
                s < step ? 'bg-[#0A7A70] border-[#0A7A70]' :
                s === step ? 'bg-white border-[#0A7A70] shadow-md shadow-[#0A7A70]/30' :
        ...
        <div className="bg-[#F8FAFC] rounded-2xl border border-[#E2E8F0] p-8 lg:p-10">
          ...
          {step === 1 && (
            <div>
                ...
                {billOptions.map(b => (
                  <button key={b} onClick={() => setForm(f => ({...f, monthlyBill: b}))}
                    className={`px-5 py-4 rounded-xl border-2 font-inter font-medium text-sm text-left transition-all duration-300 ${
                      form.monthlyBill === b
                        ? 'border-[#0A7A70] bg-[#0A7A70]/5 text-[#0A7A70]'
                        : 'border-[#E2E8F0] bg-white text-[#475569] hover:border-[#0A7A70]/40'
                    }`}>
                    {b}
                  </button>
                ))}
                    ...
                    {propertyTypes.map(t => (
                      <button key={t} onClick={() => setForm(f => ({...f, propertyType: t}))}
                        className={`px-4 py-2 rounded-lg border text-xs font-inter font-medium transition-all duration-200 ${
                          form.propertyType === t ? 'border-[#0A7A70] bg-[#0A7A70]/10 text-[#0A7A70]' : 'border-[#E2E8F0] text-[#475569] hover:border-[#0A7A70]/40'
                        }`}>
                        {t}
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {systemTypes.map(t => (
                      <button key={t} onClick={() => setForm(f => ({...f, systemType: t}))}
                        className={`px-4 py-2 rounded-lg border text-xs font-inter font-medium transition-all duration-200 ${
                          form.systemType === t ? 'border-[#0A7A70] bg-[#0A7A70]/10 text-[#0A7A70]' : 'border-[#E2E8F0] text-[#475569] hover:border-[#0A7A70]/40'
                        }`}>
                        {t}
                      </button>
                    ))}
              ...
              <button
                ...
                className="flex items-center gap-2 w-full justify-center py-4 rounded-xl bg-[#0A7A70] text-white font-jakarta font-bold disabled:opacity-40 hover:bg-[#0A7A70]/90 transition-all duration-300"
              >
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <input required value={form.city} onChange={e => setForm(f => ({...f, city: e.target.value}))}
                    className="w-full px-4 py-3 rounded-xl border border-[#E2E8F0] bg-white font-inter text-sm focus:outline-none focus:ring-2 focus:ring-[#0A7A70]/30 focus:border-[#0A7A70]"
                ...
                <div>
                  ...
                  <textarea value={form.message} onChange={e => setForm(f => ({...f, message: e.target.value}))}
                    rows={3} className="w-full px-4 py-3 rounded-xl border border-[#E2E8F0] bg-white font-inter text-sm focus:outline-none focus:ring-2 focus:ring-[#0A7A70]/30 focus:border-[#0A7A70] resize-none"
                    placeholder="Any specific requirements or questions?" />
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setStep(1)} className="flex items-center gap-2 px-6 py-4 rounded-xl border-2 border-[#E2E8F0] text-[#475569] font-jakarta font-semibold text-sm hover:border-[#0A7A70]/30 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button onClick={() => setStep(3)} disabled={!form.city}
                  className="flex-1 flex items-center justify-center gap-2 py-4 rounded-xl bg-[#0A7A70] text-white font-jakarta font-bold disabled:opacity-40 hover:bg-[#0A7A70]/90 transition-all duration-300">
                  Continue <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          ...
          {step === 3 && (
            <div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep(2)} className="flex items-center gap-2 px-6 py-4 rounded-xl border-2 border-[#E2E8F0] text-[#475569] font-jakarta font-semibold text-sm hover:border-[#0A7A70]/30 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
